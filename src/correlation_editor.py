...
class CorrelationEditor:
    ...
    def __init__(self, window, imgui_context, imgui_impl):
        ...
        if True:
            ...
            pxd_icon_no_interp = np.asarray(Image.open("src/icons/icon_ninterp_256.png")).astype(np.float32) / 255.0
            ...
            pxd_icon_linterp = np.asarray(Image.open("src/icons/icon_linterp_256.png")).astype(np.float32) / 255.0
            ...
            pxd_icon_close = np.asarray(Image.open("src/icons/icon_close_256.png")).astype(np.float32) / 255.0
    ...
    def init_toolkit():
        ...
        toolimpls = list()
        tool_source_files = glob.glob("src/ceplugins/*.py")
...
class Renderer:
    def __init__(self):
        self.quad_shader = Shader("src/shaders/ce_quad_shader.glsl")
        self.border_shader = Shader("src/shaders/ce_border_shader.glsl")
        self.gizmo_shader = Shader("src/shaders/ce_gizmo_shader.glsl")
...
class EditorGizmo:
    ...
    @staticmethod
    def init_textures():
        icon_scale = np.asarray(Image.open("src/icons/icon_scale_256.png")).astype(np.float) / 255.0
        icon_rotate = np.asarray(Image.open("src/icons/icon_rotate_256.png")).astype(np.float) / 255.0
        icon_pivot = np.asarray(Image.open("src/icons/icon_pivot_256.png")).astype(np.float) / 255.0
        icon_location = np.asarray(Image.open("src/icons/icon_location_256.png")).astype(np.float) / 255.0
        icon_scale[:, :, 0:2] = 1.0
        icon_rotate[:, :, 0:2] = 1.0
        icon_pivot[:, :, 0:2] = 1.0
        icon_location[:, :, 0] = 1.0
